...
import { Bot, Loader2, Calendar, LayoutGrid, AlertTriangle, Eye, GitMerge, Check, X, Sparkles, BookOpen, Trash2 } from 'lucide-react';
...
import { strictAudit, runUnifiedHealthCheck, applyDirtyDataFix, applyCellFix, applyNameFix, unmapRowsToNewFormat, getAuditErrorCount, type HealthReport, type HealthIssue } from '../utils/validateData';
...
function AuditReportCard({
  ...
  onApplyCellFix: (rowIndex: number, colKey: string, newValue: string) => void;
  onIgnore: () => void;
                    ...
                    {isEditing && (
                      <div className="flex gap-1 mt-1">
                        <input
                          ...
                          autoFocus
                        />
                        <button type="button" onClick={() => { onApplyCellFix(c.rowIndex, c.colKey, editValue); setEditingCell(null); }} className="text-xs px-2 py-1 rounded bg-indigo-500 text-white">确定</button>
                      </div>
                    )}
                  </li>
                );
...
export function AgentSidebar({
  ...
  /** 离开 AUDIT_REPORT / HEALTH_CHECK 时清除画布体检预览 */
  useEffect(() => {
  ...
  const applySkillToDiffPreview = useCallback((skill: SavedSkill) => {
    ...
    const mapped = mapNewRowsToBase(mergeSourceRows, skill.mapping);
    ...
    onTaskChange('DIFF_PREVIEW');
    onApplySkill?.(skill);
  }, [baseRows, newRows, mapNewRowsToBase, onTaskChange, onApplySkill, onBaselineCapture]);
    ...
    <div className="flex flex-col gap-4">
      ...
      {task === 'AUDIT_REPORT' && healthReport && (
        <AuditReportCard
          ...
          onApplyNameFix={(rowIndex, _oldName, newName) => {
          ...
          onApplyCellFix={(rowIndex, colKey, newValue) => {
