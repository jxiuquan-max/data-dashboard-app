...
export default function App() {
  ...
  /** 多表流水线：每轮合并的阶段，如 [{ name: '底表', status: 'completed' }, { name: '库存表', status: 'in_progress' }] */
  const [pipelineSteps, setPipelineSteps] = useState<Array<{ name: string; status: 'completed' | 'in_progress' }>>([]);
  ...
  /** 技能实验室：更新技能（自我进化） */
  const handleUpdateSkill = useCallback(async (skill: SavedSkill) => {
    try {
      const res = await fetch(`${FINAL_API_URL}/skills`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleChainNextTable = useCallback((rows: Record<string, string | null>[], completedStepName?: string) => {
    setFixerRows(rows);
    setBaselineRows(null);
    setNewRows([]);
    setCleanedExtraData(null);
    ...
    setPipelineSteps((prev) => {
      const next = prev.length === 0
        ? [{ name: '底表', status: 'completed' as const }, { name: completedStepName ?? '表1', status: 'completed' as const }, { name: '下一张表', status: 'in_progress' as const }]
        : [...prev.map((s, i) => i === prev.length - 1 ? { ...s, name: completedStepName ?? s.name, status: 'completed' as const } : s), { name: `表${prev.length + 1}`, status: 'in_progress' as const }];
      return next;
    });
